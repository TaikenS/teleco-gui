...
export function useAudioReceiverController() {
  ...
  useEffect(() => {
    ...
    const savedSignalPort = window.localStorage.getItem(STORAGE_KEYS.signalingPort);
    if (savedSignalPort) setSignalingPort(savedSignalPort);

    const legacySignalUrl = window.localStorage.getItem(STORAGE_KEYS.signalingWsUrlLegacy);
    if (legacySignalUrl) {
  ...
  useEffect(() => {
    window.localStorage.setItem(STORAGE_KEYS.signalingIpAddress, signalingIpAddress);
  }, [signalingIpAddress]);

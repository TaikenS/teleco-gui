...
export function normalizeTargetId(raw: string): string {
    // mqttClient.ts convention examples:
    // - rover003 -> ROVER-003
    // - teleco001 -> teleco-001
    const mRover = raw.match(/^rover(\d+)$/i);
    if (mRover) return `ROVER-${mRover[1].padStart(3, "0")}`;

    const mTeleco = raw.match(/^teleco(\d+)$/i);
    if (mTeleco) return `teleco-${mTeleco[1].padStart(3, "0")}`;
...
export function startAudioMotionPublisher(params: {
    ...
    const mouthIntervalMs = Math.max(20, Math.round(1000 / (params.mouthHz ?? 15)));

...
export default class AudioVowelProcessFormant {
    ...
    // 元コードにあったが未使用なので残しているだけ（必要なら削除OK）
    public time_wait_start = true;
    public time_waiting_start: ReturnType<typeof setTimeout> | null = null;

    public debug = false;

    constructor(opts?: Partial<Pick<AudioVowelProcessFormant, "LPC_ORDER" | "samplingRate" | "bufferSize" | "VOWEL_WINDOW">>) {
        if (opts?.LPC_ORDER != null) this.LPC_ORDER = opts.LPC_ORDER;
        if (opts?.samplingRate != null) this.samplingRate = opts.samplingRate;
        if (opts?.bufferSize != null) this.bufferSize = opts.bufferSize;
        if (opts?.VOWEL_WINDOW != null) this.VOWEL_WINDOW = opts.VOWEL_WINDOW;
    ...
    public analyzeData = (buffer: SampleBuffer): void => {
        ...
        if (ave > this.th_isSpeaking) {
            ...
            // 発話開始
            if (!this.timer_isSpeaking) this.actionstart("start");

...
function resolveDefaultOrigin(): string {
  ...
  if (typeof window !== "undefined") {
    ...
    const host = explicitPort ? `${window.location.hostname}:${explicitPort}` : window.location.host;
    return `${proto}://${host}`;
  }

  const port = explicitPort || process.env.SIGNAL_PORT || process.env.PORT || "3000";
